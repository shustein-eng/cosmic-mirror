...
import { HANDWRITING_SYSTEM_PROMPT } from '@/lib/claude/prompts'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { lens_input_id, profile_id, image_path, image_bucket } = await req.json()
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: 'text',
              text: 'Please analyze this handwriting sample using Andrea McNichol\'s trait-stroke method and Bart Baggett\'s graphology framework. Analyze slant, pressure, size, spacing, baseline, and specific letter formations. Produce a detailed personality analysis as structured JSON.',
            },
          ],
        },
      ],
    ...
    try {
      ...
      analysisResult = { summary: responseText, traits: [], notable_features: [], growth_indicators: [], methodology_note: 'Graphology analysis.' }
    }

    const { error: saveError } = await supabase
      .from('lens_inputs')
      .update({
        input_data: { image_path, image_bucket },
        image_path,
