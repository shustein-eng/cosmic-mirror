...
import { IRIDOLOGY_SYSTEM_PROMPT } from '@/lib/claude/prompts'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!imageRes.ok) return NextResponse.json({ error: 'Failed to fetch image' }, { status: 500 })
    ...
    const message = await client.messages.create({
      ...
      max_tokens: 1800,
      system: IRIDOLOGY_SYSTEM_PROMPT,
      messages: [
        {
          role: 'user',
          content: [
            ...
            {
              type: 'text',
              text: 'Please analyze this iris image using the Rayid constitutional typing method and Bernard Jensen\'s iridology framework. Identify the constitutional type (Stream, Jewel, Flower, Shaker, or Mixed), assess the color constitution, and analyze visible structural features including fiber density, nerve rings, pigmentation, lacunae, and collarette. Produce a detailed personality analysis as structured JSON. Focus only on personality and character — no health claims.',
            },
          ],
        },
      ],
    ...
    try {
      ...
      analysisResult = { summary: responseText, traits: [], notable_features: [], growth_indicators: [], methodology_note: 'Iridology analysis.' }
    }
